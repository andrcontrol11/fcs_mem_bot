...
def send_welcome(message):
    ...
    info message
    """
    bot.reply_to(message, "У данного бота много функций:\n Во-первых, бот умеет скидивать в чат рандомный мем/фото кота и тд, во-вторых, в нем есть много мемов для ВП(важных переговоров), которыми пользователь так же может воспользоваться в чате, просто обратившись к боту с помошью @Fcs_mem_bot, в-третьих, в чате с ботом встроена функция эхо(отвечает на фаше сообщение тем же сообщение) + есть две особенные комманда( для первой просто попробуйте поздороваться с ботом, а вот для второй надо узнать фамилию любимого преподавателя моего одногруппника - @nardzhiev и написать ее на русском с большой буквы)")
...
def query_text(query):
    ...
    bot.answer_inline_query(query.id, [r_text, r_pic3, r_pic4, r_pic5, r_pic6, r_pic7, r_pic8, r_pic9, r_pic10, r_pic11, r_pic12, r_pic13, r_pic14, r_pic15, r_pic16, r_pic17, r_pic18, r_pic19, r_pic20, r_pic21, r_pic22, r_pic2, r_gif, r_gif2, r_gif3], cache_time=1,is_personal=True)
...
def echo_all(message):
    ...
    if message.text == "fcs":
    	...
    	with open("2M44EqK.jpg","rb") as zobnin:
            ...
            bot.send_message(message.chat.id, "О господи, тебе было не лень написать Никите, ну ладно, тогда вот твоя награда - уставший Зобнин")
    else:
